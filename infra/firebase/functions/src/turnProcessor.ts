...
import { z } from 'zod';
import { SkateChallenge } from '../../../../packages/utils/src/types'; // Adjust import path as needed based on monorepo structure
...
export const processTurn = functions.https.onCall(async (request) => {
  if (!request.auth) throw new functions.https.HttpsError('unauthenticated', 'Log in, bro.');

  const { challengeId, action, clipUrl, result } = TurnSchema.parse(request.data);
  const uid = request.auth.uid;

  return db.runTransaction(async (transaction) => {
    const challengeRef = db.doc(`challenges/${challengeId}`);
    const snap = await transaction.get(challengeRef);
    if (!snap.exists) throw new functions.https.HttpsError('not-found', 'Challenge gone.');

    const c = snap.data() as SkateChallenge;
    if (!c.players.includes(uid)) throw new functions.https.HttpsError('permission-denied', 'Not your game.');
    ...
    if (action === 'SET') {
      if (c.status !== 'active' || c.currentSetter !== uid)
        throw new functions.https.HttpsError('failed-precondition', 'Not your turn to set.');
    ...
    // === ACTION: ATTEMPT (reply) ===
      ...
      if (c.status !== 'active' || c.currentSetter !== uid || !c.currentTrick)
        throw new functions.https.HttpsError('failed-precondition', 'No trick to land.');
    ...
    // === ACTION: JUDGE (only setter can judge) ===
      ...
      if (c.status !== 'judging' || c.currentTrick?.setterUid !== uid)
        throw new functions.https.HttpsError('permission-denied', 'Only setter judges.');
      ...
      if (!landed) {
        ...
        newLetters[attempt.attempterUid] = current + 'SKATE'.charAt(current.length);
      }

      const winner = newLetters[attempt.attempterUid]?.length === 5 ? c.currentTrick!.setterUid : undefined;

      transaction.update(challengeRef, {
        status: winner ? 'finished' : 'active',
        winner,
        letters: newLetters,
        currentSetter: landed ? attempt.attempterUid : c.currentTrick!.setterUid, // winner sets next
