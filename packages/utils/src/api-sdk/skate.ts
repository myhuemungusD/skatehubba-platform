 ...
 */
export const joinSkateChallenge = async (
  gameId: string,
  opponentUid: string,
) => {
  const db = getFirestore();
...
export const submitTurnResult = async (
  ...
  if (game.currentTurnType === "setTrick") {
    if (result === "landed") {
      ...
      updates.currentTurnUid =
        game.opponentUid === userUid ? game.challengerUid : game.opponentUid;
  ...
  // B. DEFENSE ATTEMPTING TO MATCH
    ...
    game.currentTurnType === "judgeAttempt"
  ) {
