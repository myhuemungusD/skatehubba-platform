...
// Add version headers
export function versionHeaderMiddleware(req: Request, res: Response, next: NextFunction) {
...
export class GracefulShutdown {
  ...
  initiateShutdown() {
    ...
    console.log(`⏱️  Graceful shutdown initiated. Active connections: ${this.activeConnections.size}`);
  }
  ...
  async waitForConnections(timeoutMs = 30000): Promise<void> {
    ...
    if (this.activeConnections.size > 0) {
      console.warn(`⚠️  Forcing shutdown with ${this.activeConnections.size} active connections`);
    }
  }
}
