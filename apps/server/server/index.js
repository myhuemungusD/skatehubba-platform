...
async function initializeDatabase() {
  try {
    ...
    console.warn(
      "⚠️  Database integration failed, running in basic mode:",
      error.message,
    );
  }
}
...
app.post("/api/subscribe", async (req, res) => {
  try {
    ...
    if (storage) {
      ...
      console.log(
        `📧 Subscriber [${requestId}]: ${firstName || "Anonymous"} <${email}> (ID: ${created.id})`,
      );
...
app.post("/api/feedback", async (req, res) => {
  try {
    ...
    console.log(
      `📝 [${requestId}] Feedback (${type || "general"}): ${message.substring(0, 50)}...`,
    );
