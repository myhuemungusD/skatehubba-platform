...
router.get('/bounds', async (req, res) => {
  try {
    ...
    const spotsInBounds = await db.select({
      ...spots,
...
router.post('/', async (req, res) => {
  try {
    ...
    const { name, description, latitude, longitude, spotType, bustFactor, hasLights, images, tags } = validation.data;
