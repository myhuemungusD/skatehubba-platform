 ...
 */
export const setCooldown = async (userId: string, challengeId?: string): Promise<void> => {
  ...
  try {
    ...
    console.log(`User ${userId} cooldown set. No re-dos until ${new Date(cooldownTimestamp.toMillis()).toISOString()}`);
  } catch (error) {
